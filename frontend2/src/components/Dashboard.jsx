...
const Dashboard = () => {
  ...
  useEffect(() => {
    const fetchUserData = async () => {
      try {
        ...
        const response = await axios.get(`http://localhost:5000/user/lookup`, {
        ...
        const activityResponse = await axios.get("http://localhost:5000/attendance/recent", {
          params: { card_id: userId }
        });
  ...
  const calculateStats = async (userId) => {
    try {
      ...
      // Fetch monthly stats
      const statsResponse = await axios.get("http://localhost:5000/attendance/get-monthly-attendance", {
        params: {
          card_id: userId,
          month: currentMonth
        }
