...
const GymDashboard = () => {
  ...
  const fetchLogs = async () => {
    setLoading(true);
    try {
      const response = await axios.get('http://localhost:5000/gym-logs', {
        params: {
  ...
  const logEntry = async (e) => {
    ...
    try {
      await axios.post('http://localhost:5000/log-gym-entry', userData);
  ...
  const logExit = async (logId) => {
    try {
      await axios.post(`http://localhost:5000/log-gym-exit/${logId}`);
  ...
  const exportCSV = async () => {
    try {
      const response = await axios.get('http://localhost:5000/export-gym-csv', {
        params: {
          start_date: startDate,
          end_date: endDate,
          activity_type: activityType || undefined
