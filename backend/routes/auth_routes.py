...
def login():
    ...
    try:
        ...
        if verification_result['match']:
            # Log successful attendance - Fixed by removing class_id which is not needed for general attendance
            cursor.execute("""
              INSERT INTO attendance_logs (card_id, status) 
               VALUES (%s, %s)
           """, (user_id, "Present"))
        # Update this SQL query in the login function
            # cursor.execute("""
            #     INSERT INTO attendance_logs (card_id, class_id, status) 
            #     VALUES (%s, NULL, %s)
            # """, (user_id, "Present"))
