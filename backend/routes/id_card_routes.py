...
def register_template():
    ...
    try:
        ...
        cursor.execute("""
            INSERT INTO templates (name, organization, template_class)
            VALUES (%s, %s, %s)
        """, (template_name, organization, template_class))
        
        template_id = cursor.lastrowid
        conn.commit()
        logger.debug(f"Template inserted successfully with ID: {template_id}")
...
def scan_id_card():
    ...
    try:
        ...
        cursor.execute("SELECT id, name, role FROM users WHERE card_id = %s", (card_id,))
