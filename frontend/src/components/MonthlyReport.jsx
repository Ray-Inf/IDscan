...
if (!card_id || !facilityId || !month) {
    ...
    try {
      const response = await axios.get('http://localhost:5000/api/get-monthly-attendance', {
params: { card_id: card_id, facility_id: facilityId, month }
  ...
  const exportAttendanceCSV = async () => {
    ...
    try {
      ...
      window.open(`http://localhost:5000/api/export-attendance-csv?facility_id=${facilityId}&start_date=${startDate}&end_date=${endDate}`, '_blank');
    } catch (err) {
