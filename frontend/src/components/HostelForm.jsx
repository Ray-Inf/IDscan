...
const HostelForm = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await axios.post("http://127.0.0.1:5000/api/hostel-wardens", formData, {
        headers: { "Content-Type": "multipart/form-data" },
      });
  
      if (response.data) {
        toast.success("Hostel Warden registered successfully!");
      }
    } catch (err) {
  ...
  return (
    <form onSubmit={handleSubmit} className="flex flex-col gap-8">
      <h1 className="text-xl font-semibold">Hostel Warden Registration</h1>
      ...
      <div className="flex justify-between flex-wrap gap-4">
        ...
        <InputField label="Hostel Name" name="hostelName" inputProps={{ required: true }} />
