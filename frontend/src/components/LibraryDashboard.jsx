...
const LibraryDashboard = () => {
  ...
  const fetchUsers = async () => {
    try {
      const response = await axios.get('http://localhost:5000/api/library-users');
  ...
  const logEntry = async (e) => {
    ...
    try {
      await axios.post('http://localhost:5000/api/log-library-entry', userData);
  ...
  const logExit = async (logId) => {
    try {
      await axios.post(`http://localhost:5000/api/log-library-exit/${logId}`);
  ...
  const exportCSV = async () => {
    try {
      const response = await axios.get('http://localhost:5000/api/export-library-csv', {
        params: {
          start_date: startDate,
          end_date: endDate
        },
