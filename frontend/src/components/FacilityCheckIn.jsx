...
const FacilityCheckIn = () => {
  ...
  const checkUserStatus = async (cardId) => {
    try {
      ...
      const { data } = await axios.get(`http://localhost:5000/api/user-status?card_id=${cardId}`);
  ...
  const handleManualUserLookup = async (cardId) => {
    ...
    try {
      const { data } = await axios.get(`http://localhost:5000/api/lookup?card_id=${cardId}`);
    ...
    try {
      ...
      if (uiState.isCheckingIn) {
        ...
        const response = await axios.post(
          `http://localhost:5000/api/log-facility-attendance`,
          apiFormData
        );
        ...
        const response = await axios.post(
          `http://localhost:5000/api/update-facility-attendance`,
          apiFormData
        );
