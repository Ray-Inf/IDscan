...
const DepartmentForm = ({ type, data, setOpen }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const endpoint = type === "create" ? "http://127.0.0.1:5000/api/departments" : `http://127.0.0.1:5000/api/departments/${data.id}`;
  ...
  return (
    <form onSubmit={handleSubmit} className="flex flex-col gap-8">
      ...
      <div className="flex justify-between flex-wrap gap-4">
        <InputField label="Department Name" name="name" defaultValue={data?.name} required />
